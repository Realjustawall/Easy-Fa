...
const norm=(p,id)=>{const x={...C.base(id),...(p||{})};x.enabled=x.enabled!==false;x.direction=['smart','rtl','ltr','site'].includes(x.direction)?x.direction:'smart';x.fontScope=x.fontScope==='all'?'all':'fa';x.sizeScope=x.sizeScope==='all'?'all':'fa';x.fontSize=Math.max(10,Math.min(36,+x.fontSize||16));x.fontWeight=[400,500,600,700].includes(+x.fontWeight)?+x.fontWeight:500;x.lineHeight=Math.max(1.2,Math.min(2.4,+x.lineHeight||1.75));x.composer=!!x.composer;if(!validFont(x.fontId))x.fontId=C.base(id).fontId;return x};
