...
async function load(){
  ...
  const s={version:C.VERSION,active,uiTheme:r.uiTheme==='light'?'light':'dark',profiles:{},customSites};
...
async function save(s){s.version=C.VERSION;s.uiTheme=s.uiTheme==='light'?'light':'dark';await chrome.storage.local.set({[C.KEY]:s});return s}
